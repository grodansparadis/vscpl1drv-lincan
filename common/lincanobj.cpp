...
void CLog::setFilter( unsigned long filter )
{
	m_filter = filter;
}
...
void CLog::setMask( unsigned long mask )
{
	m_mask = mask;
}
...
bool CLog::open( char *szFileName, unsigned long flags )
{
	char *p;
	bool rv = false;
...
#else
	strncpy( szFile, szFileName, PATH_MAX );
#endif
...
unsigned long CLog::getDataValue( const char *szData )
{
        unsigned long val;
        char *nstop;
